...
**آخر تحديث:** 2026-08-20 (Phase 7 Step E — تكلفة المشروع + القيد اليدوي)
**آخر تحديث سابق:** 2026-08-20 (Phase 7 Step D — خدمتا الإشعار الدائن والمدين)
...
| 7-E | تكلفة المشروع الشهرية (PMC) + القيد اليدوي — استكمال النمط أفقياً | ✅ **مكتمل هذا التحديث** — 180 تأكيداً جديداً. **غير موصول بالإنتاج بعد** |
| 7-F+ | أساس React / الوصل المُتحكَّم / معالجة العيوب الحيّة | 🔴 لم تبدأ — قرار المالك التالي |
...
| قيود اليومية | 🔴 قديم · ✅ **نواة القيد اليدوي مستخلَصة** (P7-E، غير موصولة) | Legacy |
...
| المشاريع · المستخلصات | 🔴 قديم · ✅ **تكلفة المشروع الشهرية مستخلَصة** (P7-E، غير موصولة) | Legacy |
...
| `application/` (`src/services/`) | 🟡 **سبع خدمات** — مشتريات · سند · مبيعات · إشعار دائن · إشعار مدين · **PMC · قيد يدوي** |
| `repositories/` | 🟡 **ثمانية عقود** (ستة بتنفيذين، واثنان بتنفيذ Firebase) |
...
## تكلفة المشروع (PMC) + القيد اليدوي — Phase 7 Step E

استكمال النمط أفقياً (Option A). خدمتان موازيتان جديدتان — **غير موصولتين بالإنتاج**؛
`createJournalForPMC` · `savePMC` · `saveJrnEntry` لم تُلمَس بحرف.

**PMC:** `domain/accounting/pmc/{resolvePMCAccounts,buildPMCJournal}.js` →
`ProjectCostPostingRepository` → `postProjectMonthlyCost.js`.
المرساة: حقل `journalEntryKey` **الموجود** على سجل التكلفة. القيد مطابق حرفياً — بما
فيه الخاصية المميّزة أن كلا السطرين يحمل `projectId` و`costCenter` معاً.

**القيد اليدوي:** نواة `saveJrnEntry` النقيّة مستخلَصة في
`domain/accounting/manual-journal/` (توسيع التوزيع التحليلي · سطور الضريبة التلقائية ·
تحويل العملة وموازنة التقريب · الحراسات المالية · بناء السجل) →
`ManualJournalPostingRepository` → `postManualJournal.js`.
⚠️ **الفارق البنيوي:** لا مستند مصدر — القيد نفسه هو المستند، فالمرساة مفتاحه.
⚠️ **يغطّي الإنشاء فقط**؛ ما لم يُنقَل (قفل الفترة · الصلاحيات · التنبيهات الذكية ·
تعديل قيد مرحّل) موثَّق صراحةً في `docs/services/manual-journal-posting.md §3`.

**BUG-004:** أُعيد التحقّق آلياً — التعريفان متطابقان بايتاً ببايت (2320 حرفاً)،
والاختبار يحرس التباعد المستقبلي. **لم يُحذَف أي تعريف** (يستلزم تعديل `public/`).

**BUG-015 جديد:** نوع تكلفة مخصّص (`custom_*`) يُسجَّل **بلا قيد محاسبي إطلاقاً** لأن
`getPMCCategoryInfo` لا تُرجع رموز حسابات للأنواع المخصّصة. مُثبَت تشغيلياً. غير مُصلَح.

### الاختبارات — Phase 7 Step E

| المجموعة | التأكيدات | الحالة |
|---|---:|---|
| `test:gm:pmc` (القديم مقابل الجديد + BUG-004 + BUG-015) | 38 | ✅ **جديد** |
| `test:gm:manualjrn` (توسيع السطور + تحويل العملة + الحراسات) | 32 | ✅ **جديد** |
| `test:svc:pmc` (idempotency · ذرّية · حقن فشل · مستأجرين) | 54 | ✅ **جديد** |
| `test:svc:manualjrn` | 56 | ✅ **جديد** |
| **الإجمالي (`test:phase7:e`)** | **180** | ✅ |

مسار الترحيل الكامل الآن (`npm run test:migration`): **1584 تأكيداً · صفر فشل**.
