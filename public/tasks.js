...
window.tasksPrintAgenda = function () { const c = document.getElementById('agendaCard'); if (!c) return; const w = window.open('', '_blank'); w.document.write(`<html dir="rtl" lang="ar"><head><meta charset="utf-8"><title>أجندة اليوم</title><style>body{font-family:'Segoe UI',Tahoma,sans-serif;padding:20px;direction:rtl}</style></head><body>${c.innerHTML}<script>setTimeout(()=>print(),200)<\/script></body></html>`); w.document.close(); };
