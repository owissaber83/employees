...
function hsInp() { return 'padding:8px;border:1.5px solid var(--hr-bd2);border-radius:8px;font-family:inherit;font-size:13px;box-sizing:border-box'; }
...
const HS_LETTER_TYPES = {
    salary_cert:    { label: '💰 تعريف بالراتب', salary: true,  color: 'var(--hr-acc)' },
    employment:     { label: '🧾 خطاب تعريف / إثبات عمل', salary: false, color: 'var(--hr-blue)' },
    salary_transfer:{ label: '🏦 تعريف لتحويل الراتب (بنك)', salary: true, color: 'var(--hr-alt)' },
    experience:     { label: '📜 شهادة خبرة', salary: false, color: 'var(--hr-warn)' },
    embassy:        { label: '🛂 تعريف للسفارة / تأشيرة', salary: true, color: 'var(--hr-danger)' }
};
...
window.renderHrLetters = function () {
    ...
    if (!hsCanManage()) { c.innerHTML = '<div class="card" style="padding:30px;text-align:center;color:var(--hr-danger)">🚫 هذه الصفحة متاحة للموارد البشرية فقط</div>'; return; }
    ...
    const statusBadge = l => l.status === 'pending'
        ? '<span style="background:#fef9e7;color:var(--hr-warn-d);padding:2px 9px;border-radius:10px;font-size:11px;font-weight:700">⏳ طلب موظف</span>'
        : '<span style="background:var(--hr-sf-ok);color:var(--hr-ok-d);padding:2px 9px;border-radius:10px;font-size:11px;font-weight:700">✅ صادر</span>';

    const listRow = l => {
        const t = HS_LETTER_TYPES[l.type] || HS_LETTER_TYPES.employment;
        const e = (window.emp || {})[l.empKey] || {};
        return `<tr style="border-bottom:1px solid var(--hr-sf2)">
            ...
            <td style="padding:8px 10px;text-align:center;white-space:nowrap">
                <button class="btn" onclick="hsPrintLetter('${l.k}')" style="font-size:11px;padding:4px 9px;background:var(--hr-sf3);color:var(--hr-pri2)">🖨️ عرض/طباعة</button>
    ...
    c.innerHTML = `<div style="padding:0 4px">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:14px">
            <div style="font-size:16px;font-weight:800;color:var(--hr-pri)">📄 خطابات الموارد البشرية</div>
        </div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">
            ${kpi('📋', 'خطابات صادرة', issued.length, 'var(--hr-blue)')}
            ${kpi('⏳', 'طلبات موظفين معلّقة', pending.length, pending.length ? 'var(--hr-warn)' : 'var(--hr-muted)')}
            ${kpi('👷', 'الموظفون', Object.keys(window.emp || {}).length, 'var(--hr-alt)')}
        </div>

        <div class="card" style="margin-bottom:16px;border-right:5px solid var(--hr-acc)">
            ...
            <div style="display:flex;gap:8px;margin-top:6px">
                ...
                <span style="font-size:11.5px;color:var(--hr-muted3);align-self:center">يُنشأ الخطاب برقم مرجعي ويُحفظ في السجل أدناه.</span>
            </div>
        </div>

        ${pending.length ? `<div class="card" style="margin-bottom:16px;border-right:5px solid var(--hr-warn)">
        ...
        <div class="card">
            ...
            ${issued.length ? `<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px">
                <thead><tr style="background:var(--hr-sf4);text-align:right;color:var(--hr-pri)"><th style="padding:8px 10px">الموظف</th><th style="padding:8px 10px">النوع</th><th style="padding:8px 10px">الجهة</th><th style="padding:8px 10px">التاريخ</th><th style="padding:8px 10px;text-align:center">الحالة</th><th style="padding:8px 10px;text-align:center">إجراء</th></tr></thead>
...
window.renderProbation = function () {
    ...
    c.innerHTML = `<div style="padding:0 4px">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:14px">
            <div style="font-size:16px;font-weight:800;color:var(--hr-pri)">⏳ فترة التجربة وتجديد العقود</div>
        </div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">
            ${kpi('⏳', 'فترات تجربة تنتهي قريباً', prob.length, prob.length ? 'var(--hr-warn)' : 'var(--hr-muted)')}
            ${kpi('📄', 'عقود تحتاج تجديداً', cons.length, cons.length ? 'var(--hr-danger)' : 'var(--hr-muted)')}
        </div>

        <div class="card" style="margin-bottom:16px;border-right:5px solid var(--hr-warn)">
            ...
            ${prob.length ? `<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px">
                ...
                <tbody>${prob.map(x => `<tr style="border-bottom:1px solid var(--hr-sf2)">
                    <td style="padding:8px 10px;font-weight:700">${hsEsc(x.e.name || '—')}${x.e.job ? `<div style="font-size:10.5px;color:var(--hr-muted)">${hsEsc(x.e.job)}</div>` : ''}</td>
                    ...
                    <td style="padding:8px 10px;text-align:center;white-space:nowrap">
                        ...
                        <button class="btn" onclick="hsExtendProbation('${x.k}')" style="font-size:11px;padding:4px 9px;background:var(--hr-sf3);color:var(--hr-pri2)">➕ تمديد</button>
                    </td>
                </tr>`).join('')}</tbody>
            </table></div>` : '<div style="color:var(--hr-acc);text-align:center;padding:18px;font-weight:600">✅ لا فترات تجربة تنتهي قريباً</div>'}
        </div>

        <div class="card" style="border-right:5px solid var(--hr-danger)">
            ...
            ${cons.length ? `<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px">
                ...
                <tbody>${cons.map(x => `<tr style="border-bottom:1px solid var(--hr-sf2)">
                    <td style="padding:8px 10px;font-weight:700">${hsEsc(x.e.name || '—')}${x.e.job ? `<div style="font-size:10.5px;color:var(--hr-muted)">${hsEsc(x.e.job)}</div>` : ''}</td>
            ...
            </table></div>` : '<div style="color:var(--hr-acc);text-align:center;padding:18px;font-weight:600">✅ لا عقود تحتاج تجديداً قريباً</div>'}
        </div>
    </div>`;
};
...
//  📢 إعلانات الشركة
// ═══════════════════════════════════════════════════════════════════════════
const HS_ANN_PRIORITY = { normal: { label: '🔵 عادي', color: 'var(--hr-pri2)' }, important: { label: '🟠 مهم', color: 'var(--hr-warn)' }, urgent: { label: '🔴 عاجل', color: 'var(--hr-danger)' } };
...
window.renderAnnouncements = function () {
    ...
    c.innerHTML = `<div style="padding:0 4px">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:14px">
            <div style="font-size:16px;font-weight:800;color:var(--hr-pri)">📢 إعلانات الشركة</div>
            <button class="btn b-g" onclick="hsOpenAnnounce()" style="font-weight:800">➕ إعلان جديد</button>
        </div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">
            ${kpi('📢', 'إعلانات فعّالة', active.length, 'var(--hr-acc)')}
            ${kpi('🗂️', 'إجمالي الإعلانات', all.length, 'var(--hr-blue)')}
        </div>
        ...
        return `<div style="background:#fff;border-radius:12px;box-shadow:0 1px 4px rgba(0,0,0,.06);overflow:hidden;border-top:3px solid ${pr.color};opacity:${off || expired ? '.6' : '1'}">
            ...
            <div style="padding:8px 14px;border-top:1px solid #f5f5f5;display:flex;gap:6px;justify-content:flex-end">
                ...
                <button class="btn" onclick="hsToggleAnnounce('${a.k}',${a.active === false})" style="font-size:11px;padding:4px 9px;background:var(--hr-sf3);color:var(--hr-pri2)">${a.active === false ? '▶️ تفعيل' : '⏸️ إيقاف'}</button>
...
function hsEnsureAnnModal() {
    ...
    d.innerHTML = `<div style="background:#fff;border-radius:14px;max-width:520px;width:100%;max-height:92vh;overflow:auto;padding:22px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3 id="hsAnnTitle" style="margin:0;color:var(--hr-acc);font-size:18px">📢 إعلان جديد</h3><button onclick="hsCloseAnnounce()" style="background:none;border:none;font-size:22px;cursor:pointer;color:#888">×</button></div>
        ...
        <div style="display:flex;gap:8px;margin-top:8px"><button class="btn b-g" onclick="hsSaveAnnounce()" style="flex:1;font-weight:800">💾 نشر</button><button class="btn" onclick="hsCloseAnnounce()" style="background:var(--hr-sf1)">إلغاء</button></div>
    </div>`;
...
function hsNitaqatBand(pct) {
    if (pct >= 40) return { label: 'بلاتيني', color: 'var(--hr-muted2)', bg: '#ecf0f1' };
    if (pct >= 30) return { label: 'أخضر مرتفع', color: 'var(--hr-ok-d)', bg: 'var(--hr-sf-ok)' };
    if (pct >= 20) return { label: 'أخضر متوسط', color: 'var(--hr-ok)', bg: 'var(--hr-sf-ok)' };
    if (pct >= 10) return { label: 'أخضر منخفض', color: '#2ecc71', bg: '#f0faf3' };
    if (pct >= 5)  return { label: 'أصفر', color: 'var(--hr-warn-d)', bg: '#fef9e7' };
    return { label: 'أحمر', color: 'var(--hr-danger)', bg: '#fdecea' };
}
...
window.renderHrAnalytics = function () {
    ...
    const kpi = (icon, label, val, col, sub) => `<div style="background:#fff;border-radius:12px;padding:14px 16px;flex:1;min-width:150px;border-top:3px solid ${col};box-shadow:0 1px 4px rgba(0,0,0,.05)"><div style="font-size:12px;color:#888">${icon} ${label}</div><div style="font-size:22px;font-weight:800;color:${col};margin-top:4px">${val}</div>${sub ? `<div style="font-size:10.5px;color:var(--hr-muted);margin-top:2px">${sub}</div>` : ''}</div>`;
    const bar = (pct, col) => `<div style="background:var(--hr-sf5);border-radius:6px;height:8px;overflow:hidden;min-width:70px"><div style="width:${Math.min(100, pct)}%;height:100%;background:${col}"></div></div>`;

    c.innerHTML = `<div style="padding:0 4px" id="hsAnalyticsPrint">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:14px">
            <div style="font-size:16px;font-weight:800;color:var(--hr-pri)">📊 تحليلات الموارد البشرية والسعودة</div>
            <button class="btn" onclick="window.print()" style="background:var(--hr-sf3);color:var(--hr-pri2);font-weight:700">🖨️ طباعة</button>
        </div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">
            ${kpi('👷', 'إجمالي الموظفين', total, 'var(--hr-blue)', `${inactive.length ? inactive.length + ' منتهية خدمتهم' : 'الكل على رأس العمل'}`)}
            ${kpi('🇸🇦', 'نسبة السعودة', satzPct + '%', band.color, `${saudis} سعودي / ${nonSaudis} غير سعودي`)}
            ${kpi('💰', 'تكلفة الرواتب/شهر', hsMoney(salaryCost), 'var(--hr-acc)', `متوسط ${hsMoney(avgSalary)} ر`)}
            ${kpi('⏳', 'متوسط مدة الخدمة', avgTenure.toFixed(1) + ' سنة', 'var(--hr-alt)')}
            ${kpi('🔄', 'معدل الدوران', turnoverPct + '%', turnoverPct > 15 ? 'var(--hr-warn)' : 'var(--hr-muted)', 'تقريبي (تراكمي)')}
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:14px;margin-bottom:16px">
            <div class="card">
                ...
                <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12.5px">
                    <thead><tr style="background:var(--hr-sf4);text-align:right;color:var(--hr-pri)"><th style="padding:7px 9px">القسم</th><th style="padding:7px 9px;text-align:center">العدد</th><th style="padding:7px 9px;text-align:center">السعودة</th><th style="padding:7px 9px;text-align:left">التكلفة/شهر</th></tr></thead>
                    <tbody>${Object.entries(byDept).sort((a, b) => b[1].n - a[1].n).map(([d, v]) => `<tr style="border-bottom:1px solid var(--hr-sf2)"><td style="padding:6px 9px;font-weight:700">${hsEsc(d)}</td><td style="padding:6px 9px;text-align:center">${v.n}</td><td style="padding:6px 9px;text-align:center">${v.n ? Math.round(v.s / v.n * 100) : 0}%</td><td style="padding:6px 9px;text-align:left">${hsMoney(v.cost)}</td></tr>`).join('') || '<tr><td colspan="4" style="padding:14px;text-align:center;color:#aaa">لا بيانات</td></tr>'}</tbody>
                </table></div>
            </div>
            <div class="card">
                <div class="c-tl">🌍 التوزيع حسب الجنسية</div>
                ${Object.entries(byNat).sort((a, b) => b[1] - a[1]).map(([n, v]) => `<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><div style="width:120px;font-size:12.5px;font-weight:600">${hsEsc(n)}</div>${bar(total ? v / total * 100 : 0, hsIsSaudi(n) ? 'var(--hr-acc)' : 'var(--hr-blue)')}<div style="font-size:12px;font-weight:700;color:#555;min-width:28px">${v}</div></div>`).join('') || '<div style="color:#aaa;text-align:center;padding:14px">لا بيانات</div>'}
            </div>
        </div>

        <div class="card" style="border-right:5px solid var(--hr-warn)">
            <div class="c-tl">📋 انتهاء الإقامات (الموظفون على رأس العمل)</div>
            <div style="display:flex;gap:12px;flex-wrap:wrap">
                ${kpi('🔴', 'منتهية', iqExp.expired, iqExp.expired ? 'var(--hr-danger)' : 'var(--hr-muted)')}
                ${kpi('🟠', 'خلال 30 يوم', iqExp.d30, iqExp.d30 ? 'var(--hr-warn)' : 'var(--hr-muted)')}
                ${kpi('🟡', 'خلال 60 يوم', iqExp.d60, iqExp.d60 ? 'var(--hr-warn-d)' : 'var(--hr-muted)')}
            </div>
            <div style="font-size:11.5px;color:var(--hr-muted3);margin-top:8px">للتفاصيل والتنبيهات الكاملة راجع صفحة «⏰ تنبيهات المستندات».</div>
        </div>
    </div>`;
};
...
// ═══════════════════════════════════════════════════════════════════════════
const HS_GOAL_TYPES = { okr: { label: '🎯 هدف (OKR)', color: 'var(--hr-alt)' }, kpi: { label: '📊 مؤشر (KPI)', color: 'var(--hr-blue)' } };
...
window.renderGoals = function () {
    ...
    if (!hsCanPerf()) { c.innerHTML = '<div class="card" style="padding:30px;text-align:center;color:var(--hr-danger)">🚫 هذه الصفحة متاحة للموارد البشرية فقط</div>'; return; }
    ...
    const kpi = (icon, label, val, col) => `<div style="background:#fff;border-radius:12px;padding:14px 18px;flex:1;min-width:150px;border-top:3px solid ${col};box-shadow:0 1px 4px rgba(0,0,0,.05)"><div style="font-size:12px;color:#888">${icon} ${label}</div><div style="font-size:22px;font-weight:800;color:${col};margin-top:4px">${val}</div></div>`;
    const bar = (pct, col) => `<div style="background:var(--hr-sf5);border-radius:6px;height:9px;overflow:hidden;flex:1;min-width:90px"><div style="width:${Math.min(100, Math.max(0, pct))}%;height:100%;background:${col};transition:width .3s"></div></div>`;
    ...
    c.innerHTML = `<div style="padding:0 4px">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:14px">
            <div style="font-size:16px;font-weight:800;color:var(--hr-pri)">🎯 الأهداف ومؤشرات الأداء (OKR/KPI)</div>
        ...
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">
            ${kpi('🎯', 'أهداف نشطة', active.length, 'var(--hr-alt)')}
            ${kpi('✅', 'مكتملة', done.length, 'var(--hr-acc)')}
            ${kpi('📈', 'متوسط الإنجاز', avgProg + '%', avgProg >= 70 ? 'var(--hr-acc)' : avgProg >= 40 ? 'var(--hr-warn)' : 'var(--hr-danger)')}
        </div>
        ...
        const scol = score >= 70 ? 'var(--hr-acc)' : score >= 40 ? 'var(--hr-warn)' : 'var(--hr-danger)';
        return `<div class="card" style="margin-bottom:12px">
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:10px">
                <div style="font-weight:800;color:var(--hr-pri);font-size:14px">👤 ${hsEsc(e.name || '—')} <span style="font-size:11px;color:var(--hr-muted);font-weight:600">${hsEsc(e.job || '')}</span></div>
            ...
            const pcol = pr >= 70 ? 'var(--hr-acc)' : pr >= 40 ? 'var(--hr-warn)' : 'var(--hr-danger)';
            const st = g.status === 'done' ? '✅' : g.status === 'cancelled' ? '🚫' : '';
            return `<div style="border:1px solid var(--hr-sf5);border-radius:9px;padding:10px 12px;margin-bottom:8px">
                <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px">
                    <div style="font-weight:700;font-size:13px;color:var(--hr-ink)">${st} <span style="color:${t.color}">${t.label.split(' ')[0]}</span> ${hsEsc(g.title || '')}${g.target ? ` <span style="font-size:11px;color:var(--hr-muted)">(الهدف: ${hsEsc(g.target)}${g.unit ? ' ' + hsEsc(g.unit) : ''})</span>` : ''}</div>
                    <div style="font-size:11px;color:var(--hr-muted)">وزن ${g.weight || 0}%${g.dueDate ? ` · ${g.dueDate}` : ''}</div>
                </div>
...
function hsEnsureGoalModal() {
    ...
    d.innerHTML = `<div style="background:#fff;border-radius:14px;max-width:540px;width:100%;max-height:92vh;overflow:auto;padding:22px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3 id="hsGoalTitle" style="margin:0;color:var(--hr-alt);font-size:18px">🎯 هدف/مؤشر جديد</h3><button onclick="hsCloseGoal()" style="background:none;border:none;font-size:22px;cursor:pointer;color:#888">×</button></div>
        ...
        <div style="display:flex;gap:8px;margin-top:8px"><button class="btn b-g" onclick="hsSaveGoal()" style="flex:1;font-weight:800">💾 حفظ</button><button class="btn" onclick="hsCloseGoal()" style="background:var(--hr-sf1)">إلغاء</button></div>
    </div>`;
    document.body.appendChild(d);
}
...
const HS_TRN_STATUS = { planned: { label: '📅 مخطّط', color: 'var(--hr-alt)' }, ongoing: { label: '🔵 جارٍ', color: 'var(--hr-blue)' }, completed: { label: '✅ مكتمل', color: 'var(--hr-acc)' }, cancelled: { label: '🚫 ملغى', color: 'var(--hr-muted)' } };
...
window.renderTraining = function () {
    ...
    c.innerHTML = `<div style="padding:0 4px">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:14px">
            <div style="font-size:16px;font-weight:800;color:var(--hr-pri)">🎓 التدريب والتطوير</div>
            <button class="btn b-g" onclick="hsOpenTrn()" style="font-weight:800">➕ دورة/شهادة</button>
        </div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">
            ${kpi('🎓', 'إجمالي الدورات', all.length, 'var(--hr-blue)')}
            ${kpi('✅', 'مكتملة', completed, 'var(--hr-acc)')}
            ${kpi('🔵', 'جارية', ongoing, ongoing ? 'var(--hr-blue)' : 'var(--hr-muted)')}
            ${kpi('💰', 'تكلفة التدريب', hsMoney(totalCost), 'var(--hr-alt)')}
        </div>
        ${expCerts.length ? `<div class="card" style="margin-bottom:14px;border-right:5px solid var(--hr-warn);background:#fef9f3">
            <div style="font-weight:800;color:var(--hr-warn-d);font-size:13.5px">⚠️ ${expCerts.length} شهادة تنتهي خلال 60 يوماً</div>
        ...
        <div class="card">
            ...
            ${rows.length ? `<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12.5px">
                <thead><tr style="background:var(--hr-sf4);text-align:right;color:var(--hr-pri)"><th style="padding:8px 9px">الموظف</th><th style="padding:8px 9px">الدورة</th><th style="padding:8px 9px">النوع</th><th style="padding:8px 9px">الفترة</th><th style="padding:8px 9px;text-align:center">الحالة</th><th style="padding:8px 9px;text-align:center">شهادة</th><th style="padding:8px 9px;text-align:center">إجراء</th></tr></thead>
                <tbody>${rows.map(t => { const st = HS_TRN_STATUS[t.status || 'planned'] || HS_TRN_STATUS.planned; return `<tr style="border-bottom:1px solid var(--hr-sf2)">
...
function hsEnsureTrnModal() {
    ...
    d.innerHTML = `<div style="background:#fff;border-radius:14px;max-width:560px;width:100%;max-height:92vh;overflow:auto;padding:22px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3 id="hsTrnTitle" style="margin:0;color:var(--hr-blue);font-size:18px">🎓 دورة/شهادة جديدة</h3><button onclick="hsCloseTrn()" style="background:none;border:none;font-size:22px;cursor:pointer;color:#888">×</button></div>
        ...
        <div style="background:#f7fbff;border:1px solid #e3eef7;border-radius:10px;padding:10px 12px;margin-bottom:10px">
            <label style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:var(--hr-blue);cursor:pointer;margin-bottom:8px"><input type="checkbox" id="hsTrnHasCert" onchange="document.getElementById('hsTrnCertRow').style.display=this.checked?'grid':'none'"> 📜 نتج عنها شهادة</label>
        ...
        <div style="display:flex;gap:8px;margin-top:8px"><button class="btn b-g" onclick="hsSaveTrn()" style="flex:1;font-weight:800">💾 حفظ</button><button class="btn" onclick="hsCloseTrn()" style="background:var(--hr-sf1)">إلغاء</button></div>
    </div>`;
    document.body.appendChild(d);
}
...
window.renderSurveys = function () {
    ...
    c.innerHTML = `<div style="padding:0 4px">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:14px">
            <div style="font-size:16px;font-weight:800;color:var(--hr-pri)">📝 استبيانات رضا الموظفين</div>
            <button class="btn b-g" onclick="hsOpenSurvey()" style="font-weight:800">➕ استبيان جديد</button>
        </div>
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:14px">
            ${kpi('📝', 'إجمالي الاستبيانات', all.length, 'var(--hr-blue)')}
            ${kpi('🟢', 'مفتوحة الآن', all.filter(s => s.active !== false).length, 'var(--hr-acc)')}
        </div>
        ...
        let scoreHtml = '';
        if (isEnps) { const sc = enps.score; const col = sc == null ? 'var(--hr-muted)' : sc >= 30 ? 'var(--hr-acc)' : sc >= 0 ? 'var(--hr-warn)' : 'var(--hr-danger)'; scoreHtml = `<div style="font-size:28px;font-weight:900;color:${col}">${sc == null ? '—' : (sc > 0 ? '+' : '') + sc}</div><div style="font-size:10px;color:var(--hr-muted)">eNPS</div>`; }
        else { const avg = hsSurveyAvg(s, resps); const col = avg == null ? 'var(--hr-muted)' : avg >= 4 ? 'var(--hr-acc)' : avg >= 3 ? 'var(--hr-warn)' : 'var(--hr-danger)'; scoreHtml = `<div style="font-size:28px;font-weight:900;color:${col}">${avg == null ? '—' : avg.toFixed(1)}</div><div style="font-size:10px;color:var(--hr-muted)">من 5</div>`; }
        const off = s.active === false;
        return `<div style="background:#fff;border-radius:12px;box-shadow:0 1px 4px rgba(0,0,0,.06);overflow:hidden;border-top:3px solid ${isEnps ? 'var(--hr-alt)' : 'var(--hr-blue)'};opacity:${off ? '.7' : '1'}">
            <div style="padding:12px 14px;border-bottom:1px solid var(--hr-sf1);display:flex;justify-content:space-between;align-items:center;gap:10px">
                <div><div style="font-weight:800;color:var(--hr-pri);font-size:14px">${hsEsc(s.title || 'استبيان')}</div><div style="font-size:10.5px;color:var(--hr-muted);margin-top:2px">${isEnps ? '💜 eNPS' : '⭐ تقييم'}${s.anonymous ? ' · مجهول' : ''}${off ? ' · مغلق' : ''}</div></div>
            ...
            <div style="padding:11px 14px;font-size:12px;color:#556">
                ...
                ${isEnps && enps.n ? `<div style="display:flex;gap:4px;height:8px;border-radius:5px;overflow:hidden;margin-top:5px"><div style="flex:${enps.prom};background:var(--hr-acc)" title="مروّجون ${enps.prom}"></div><div style="flex:${enps.pass};background:#f1c40f" title="محايدون ${enps.pass}"></div><div style="flex:${enps.det};background:var(--hr-danger)" title="منتقدون ${enps.det}"></div></div><div style="font-size:10px;color:var(--hr-muted);margin-top:3px">🟢 ${enps.prom} مروّج · 🟡 ${enps.pass} محايد · 🔴 ${enps.det} منتقد</div>` : ''}
            </div>
            <div style="padding:8px 14px;border-top:1px solid #f5f5f5;display:flex;gap:6px;justify-content:flex-end">
                <button class="btn" onclick="hsSurveyResults('${s.k}')" style="font-size:11px;padding:4px 9px;background:var(--hr-sf3);color:var(--hr-pri2)">📊 النتائج</button>
...
window.hsSurveyResults = function (sid) {
    ...
    if (s.type === 'enps') {
        ...
        body = `<div style="text-align:center;padding:10px"><div style="font-size:40px;font-weight:900;color:${e.score == null ? 'var(--hr-muted)' : e.score >= 0 ? 'var(--hr-acc)' : 'var(--hr-danger)'}">${e.score == null ? '—' : (e.score > 0 ? '+' : '') + e.score}</div><div style="color:#888">صافي نقاط ترشيح الموظف (eNPS) · ${e.n} رد</div></div>`;
    } else {
        body = (s.questions || []).map(q => {
            const vals = resps.map(r => parseFloat((r.answers || {})[q.id])).filter(v => !isNaN(v));
            const avg = vals.length ? (vals.reduce((a, b) => a + b, 0) / vals.length) : null;
            const col = avg == null ? 'var(--hr-muted)' : avg >= 4 ? 'var(--hr-acc)' : avg >= 3 ? 'var(--hr-warn)' : 'var(--hr-danger)';
            return `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--hr-sf2)"><span style="font-size:13px">${hsEsc(q.text)}</span><b style="color:${col}">${avg == null ? '—' : avg.toFixed(1)} / 5</b></div>`;
        }).join('');
    }
    const comments = resps.filter(r => (r.comment || '').trim()).map(r => `<div style="background:#f8fafc;border-radius:8px;padding:8px 11px;margin-bottom:6px;font-size:12px;color:#556">💬 ${hsEsc(r.comment)}${!s.anonymous && r.empName ? `<div style="font-size:10px;color:var(--hr-muted);margin-top:2px">${hsEsc(r.empName)}</div>` : ''}</div>`).join('');
...
function hsEnsureSurveyModal() {
    ...
    d.style.cssText = 'display:none;position:fixed;inset:0;z-index:8000;background:rgba(0,0,0,.45);align-items:center;justify-content:center;padding:16px';
    d.innerHTML = `<div style="background:#fff;border-radius:14px;max-width:540px;width:100%;max-height:92vh;overflow:auto;padding:22px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3 style="margin:0;color:var(--hr-alt);font-size:18px">📝 استبيان جديد</h3><button onclick="hsCloseSurvey()" style="background:none;border:none;font-size:22px;cursor:pointer;color:#888">×</button></div>
        ...
        <div style="display:flex;gap:8px;margin-top:4px"><button class="btn b-g" onclick="hsSaveSurvey()" style="flex:1;font-weight:800">📤 نشر الاستبيان</button><button class="btn" onclick="hsCloseSurvey()" style="background:var(--hr-sf1)">إلغاء</button></div>
    </div>`;
    document.body.appendChild(d);
}
...
window.hsAnswerSurvey = function (sid) {
    ...
    let qHtml = '';
    if (s.type === 'enps') {
        qHtml = `<div style="font-size:13px;font-weight:700;color:var(--hr-ink);margin-bottom:8px">${hsEsc(HS_ENPS_Q)}</div>
        <div id="hsAnsEnps" style="display:flex;flex-wrap:wrap;gap:5px;margin-bottom:12px">${Array.from({ length: 11 }, (_, i) => `<button type="button" onclick="hsPickEnps(${i},this)" style="width:34px;height:34px;border:1.5px solid var(--hr-bd2);background:#fff;border-radius:8px;cursor:pointer;font-weight:800;font-family:inherit">${i}</button>`).join('')}</div>`;
    } else {
        qHtml = (s.questions || []).map(q => `<div style="margin-bottom:12px"><div style="font-size:13px;font-weight:700;color:var(--hr-ink);margin-bottom:6px">${hsEsc(q.text)}</div><div style="display:flex;gap:6px" data-qid="${q.id}">${[1, 2, 3, 4, 5].map(n => `<button type="button" onclick="hsPickRate('${q.id}',${n},this)" style="flex:1;height:38px;border:1.5px solid var(--hr-bd2);background:#fff;border-radius:8px;cursor:pointer;font-weight:800;font-family:inherit">${n}</button>`).join('')}</div></div>`).join('');
    }
    window._hsAns = { surveyId: sid, answers: {} };
    hsModal(`📝 ${hsEsc(s.title || 'استبيان')}`, `${s.description ? `<div style="font-size:12px;color:#888;margin-bottom:10px">${hsEsc(s.description)}</div>` : ''}${qHtml}<div style="margin:6px 0 12px"><textarea id="hsAnsComment" rows="2" placeholder="تعليق (اختياري)" style="width:100%;${hsInp()};resize:vertical"></textarea></div><button class="btn b-g" onclick="hsSubmitSurvey()" style="width:100%;font-weight:800">📤 إرسال</button>${s.anonymous ? '<div style="font-size:10.5px;color:var(--hr-muted);text-align:center;margin-top:6px">🔒 هذا الاستبيان مجهول — لن تُعرض هويتك في النتائج</div>' : ''}`);
};
window.hsPickEnps = function (v, btn) { window._hsAns.answers.enps = v; Array.from(btn.parentElement.children).forEach(b => { b.style.background = '#fff'; b.style.color = '#000'; b.style.borderColor = 'var(--hr-bd2)'; }); btn.style.background = 'var(--hr-alt)'; btn.style.color = '#fff'; btn.style.borderColor = 'var(--hr-alt)'; };
window.hsPickRate = function (qid, v, btn) { window._hsAns.answers[qid] = v; Array.from(btn.parentElement.children).forEach(b => { b.style.background = '#fff'; b.style.color = '#000'; b.style.borderColor = 'var(--hr-bd2)'; }); btn.style.background = 'var(--hr-acc)'; btn.style.color = '#fff'; btn.style.borderColor = 'var(--hr-acc)'; };
