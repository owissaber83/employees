...
**خدمة ترحيل فاتورة المبيعات اكتملت ومُختبَرة (Phase 7 Step C) — غير موصولة
بالإنتاج.** ثلاث خدمات ترحيل جاهزة الآن (مشتريات · سند · مبيعات) على نفس
المعمارية. المسارات التالية بانتظار قرار المالك، لا يُبدأ أيّ منها بلا أمر
صريح (كل واحد بوّابة موافقة منفصلة):

**أ) استكمال النمط أفقياً** — إشعارات دائن/مدين (أقرب المتبقّي شبهاً بالمبيعات:
مرتجع مبيعات يُنتج حركة مخزون **داخلة** — accounting.js:16164)، ثم PMC، ثم
القيد اليدوي (الأعقد — موافقة فرعية إضافية).

**ب) وصل مُتحكَّم بالإنتاج** — أيّ من الخدمات الثلاث، خلف عَلَم ميزة، مع خطة
تراجع فورية (`git revert` وحده كافٍ — لا تغيير مخطّط). **قرار مستقلّ تماماً**
عن اكتمال الاختبارات. ⚠️ لفاتورة المبيعات تحديداً، الوصل يعني أيضاً حسم
القرارات المعلّقة **D1** (هل يُنشئ النظام حساباً قياسياً تلقائياً؟) و**D2**
(هل يُنشأ قيد تكلفة بضاعة مباعة؟) — راجع `docs/services/sales-invoice-posting.md §4`.
كما أن الخدمة **لا تُطبّق ضوابط الواجهة** (الصلاحيات · حد الاعتماد · الإيقاف
والحد الائتمانيان · قفل الفترة) — تبقى مسؤولية المهايئ عند الوصل.

**ج) الانتقال إلى Phase 8 (أساس React)** — تأجيل توسيع الخدمات، بناء طبقة
العرض فوق ما استقرّ (النطاق + المستودعات + الخدمات الثلاث) كنموذج يُثبت
الجزيرة الأولى قبل تكرار النمط عبر بقية المسارات.

**لا توصية مفروضة** — الثلاثة صحيحة هندسياً، والاختيار يعتمد على أولوية عمل
لا على معيار تقني حاسم. ما تغيّر عن آخر مرّة: المبيعات لم تعد عائقاً — كانت
مؤجَّلة بانتظار حسم BUG-006، وقد **عُزل** بقرار «لا إنشاء حسابات في الخدمة»
بدل انتظار حسمه.
...
5. اقرأ `docs/services/sales-invoice-discovery.md` قبل أي عمل على مسار مبيعات أو مخزون — يحوي جدول التتبّع الكامل ومجموعة الكتابة الفعلية (3 + 2N عملية في القديم) وإجابات §5 عن `ensureStdAccount`
