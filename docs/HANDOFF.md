...
**خدمتا الإشعار الدائن والمدين اكتملتا ومُختبَرتان (Phase 7 Step D) — غير موصولتين
بالإنتاج.** خمس خدمات ترحيل جاهزة الآن على نفس المعمارية: مشتريات · سند · مبيعات ·
إشعار دائن · إشعار مدين. المسارات التالية بانتظار قرار المالك، لا يُبدأ أيّ منها بلا
أمر صريح:

**أ) استكمال النمط أفقياً** — المتبقّي: PMC (`createJournalForPMC` — مُعرَّفة مرّتين،
BUG-004) ثم القيد اليدوي (الأعقد — موافقة فرعية إضافية).

**ب) وصل مُتحكَّم بالإنتاج** — أيّ من الخدمات الخمس، خلف عَلَم ميزة، مع `git revert`
كخطة تراجع. ⚠️ للإشعارين تحديداً، الوصل يستلزم:
1. حسم **D3** (عكس الاحتجاز 1131 والدفعة المقدمة 2150 في الإشعار الدائن).
2. حسم **D4** (أساس تقييم مخزون الإشعار المدين: سعر السطر أم المتوسط المتحرّك).
3. مهايئ يولّد `noteKey` **مرّة واحدة** عند فتح الشاشة ويعيد استخدامه في كل محاولة —
   هذا شرط صحّة ضمان Idempotency كلّه.
4. مهايئ يمرّر `returnQuantities` صراحةً بدل DOM.
5. ضوابط الواجهة (الصلاحيات · قفل الفترة · التأكيد) تبقى مسؤولية المهايئ.

**ج) معالجة العيوب الحرجة على المسار الحيّ** — صار لدينا الآن تسعة عيوب موثَّقة
(BUG-006…014)، ثلاثة منها تُنتج **انفصالاً بين شاشة رصيد الطرف ودفتر الأستاذ**
(BUG-007 · BUG-010 · BUG-013). كلها مُثبتة باختبارات تشغيلية على الشفرة الحيّة، وكلها
تنتظر موافقة صريحة. هذا المسار **لا يحتاج React ولا وصلاً** — إصلاحات موضعية بأثر
مالي مباشر.

**د) الانتقال إلى Phase 8 (أساس React)** — البناء فوق ما استقرّ.

**لا توصية مفروضة.** ما تغيّر عن آخر مرّة: مسار الإشعارات لم يعد فجوة، وظهرت خمسة
عيوب جديدة أخطرها BUG-010 (إشعار بلا قيد) وBUG-014 (التزام مخفي تجاه العميل).
...
5. اقرأ `docs/services/credit-note-discovery.md` و`debit-note-discovery.md` قبل أي عمل على الإشعارات — يحويان جدول التتبّع الكامل وتوصيف العيوب الخمسة الجديدة
6. اقرأ `docs/services/credit-note-idempotency.md §4` قبل أي وصل — «تكرار الطلب بمفتاح جديد ليس تكراراً»، وهذا حدّ بنيوي لا عيب تنفيذ
7. اقرأ `docs/services/sales-invoice-discovery.md` قبل أي عمل على مسار مبيعات أو مخزون — يحوي جدول التتبّع الكامل ومجموعة الكتابة الفعلية (3 + 2N عملية في القديم) وإجابات §5 عن `ensureStdAccount`
6. اقرأ `docs/services/sales-invoice-inventory.md` قبل أي مسار يمسّ المخزون — **لا قيد تكلفة بضاعة مباعة في النظام إطلاقاً** (جرد دوري لا مستمر)؛ افتراض العكس يُنتج قيوداً لا وجود لها في القديم

**إن اختير (أ) للإشعارات الدائنة/المدينة:** ابدأ من
`docs/services/sales-invoice-*.md` — الإشعار الدائن يعكس نفس المسار
(`accounting.js:16162`) بحركة مخزون **داخلة** بتكلفة المتوسط المتحرّك نفسها،
فـ`domain/inventory/movingAverage.js` تُعاد استخدامها كما هي.

**إن اختير (ب):** لا يبدأ بلا موافقة صريحة منفصلة عن موافقة أي مرحلة سابقة.

**سؤال مفتوح بانتظار قرار المالك:**
- **(D1)** `ensureStdAccount` تُنشئ حسابات في الشجرة كأثر جانبي للترحيل بلا
  موافقة، وقد تُنشئ **نسختين** بنفس الرمز تحت تزامن (BUG-006) — يبقى التصميم
  أم يتحوّل إلى اقتراح يتطلّب تأكيداً + فهرس فرادة (**تغيير مخطّط**)؟
  ⚠️ **لم يعد عائقاً للاستخلاص**: خدمة المبيعات (Phase 7-C) عزلت نفسها عنه
  بقرار «لا إنشاء حسابات إطلاقاً — رفض صريح بدلاً منه». يبقى مفتوحاً لأنه
  شرط **وصل** لا شرط بناء.
- **(D2)** لا يوجد في النظام أي قيد مخزون أو تكلفة بضاعة مباعة عند البيع
  (جرد دوري لا مستمر — أُكِّد بقراءة الشفرة في Phase 7-C). هل يبقى كذلك أم
  يتحوّل إلى جرد مستمر؟ **قرار محاسبي بحت**، خارج صلاحية الترحيل.
- **(D3)** الإشعار الدائن **لا يعكس** الاحتجاز (1131) ولا الدفعة المقدمة (2150) حتى
  لو حملتهما الفاتورة الأصلية (Phase 7-D). يبقى كذلك أم يُعكسان؟ **قرار محاسبي**.
- **(D4)** الإشعار المدين يُخرج المخزون **بسعر السطر** بينما التقييم الجاري بالمتوسط
  المرجّح المتحرّك ⇒ انحرافٌ في قيمة المخزون عند اختلاف السعرين. توحيد السياستين
  يغيّر قيمة المخزون في القوائم المالية — **قرار مالك، لم يُتَّخذ**.
